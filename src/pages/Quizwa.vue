    ...
    <section class="quiz-section">
      <div id="quiz" class="quiz">
        <div
          ...
          :style="{
            width:
              ((questionIndex + startProgress) / quizwa.questions.length) *
                100 +
              '%',
          }"
        ></div>
        <div class="endAnswers" v-if="quizwa.extraInfo.answersAtEnd">
          <div class="quiz-head" v-if="end">
            <div
              v-for="(question, index) in quizwa.questions"
              :key="question.text"
        ...
        <div class="popup" v-if="quizwa.extraInfo.answersAtOnse">
          <div
            ...
            <transition name="fade" mode="out-in">
              ...
              <div class="quiz-head" v-else>
                ...
                <div class="quiz-head__image">
                  <div
                    :style="{
                      'background-image': 'url(' + quizwa.plug.image.end + ')',
                    }"
                  ></div>
                </div>
                <div class="quiz-footer">
                  <button @click="restart" class="quiz-footer__button">
                    {{ quizwa.plug.buttontext.end }}
                  </button>
                  <button
                    @click="end = !end"
                    v-if="quizwa.extraInfo.answersAtEnd"
                    class="quiz-footer__button"
                  >
                    Answers
                  </button>
          ...
          <div class="quiz-main" ref="main" v-else>
            <div class="quiz-head">
              <div class="quiz-head__question-text">
                <p>
                  {{ questionIndex + 1 }}.
                  {{ quizwa.questions[questionIndex].text }}
                </p>
              </div>
              <transition name="fade">
                <div
                  class="quiz-head__image"
                  :key="quizwa.questions[questionIndex].image"
                >
                  <div
                    :style="{
                      'background-image':
                        'url(' + quizwa.questions[questionIndex].image + ')',
                    }"
                  ></div>
                </div>
              </transition>
            ...
            <div
              ...
              v-if="!quizwa.questions[questionIndex].manyAnswers"
            >
              <div
                v-for="(response, index) in quizwa.questions[questionIndex]
                  .responses"
                :key="response.text"
                class="quiz-body__response-text"
              >
            ...
            <div
              class="quiz-body"
              :data-index="questionIndex"
              v-else-if="quizwa.questions[questionIndex].manyAnswers"
            >
              <div
                v-for="(response, index) in quizwa.questions[questionIndex]
                  .responses"
...
export default {
  data() {
    return {
      ...
      selectedResponses: [],
      mode: "start",
      questionLength: quizwa.questions.length,
      totalText: "",
      popup: false,
      correct: "",
      correctText: "",
  ...
  methods: {
    ...
    totalScoreText: function () {
      ...
      if (this.correctAnswerSum <= responseLevel) {
        this.totalText = quizwa.plug.totalText.raw;
      } else if (
        responseLevel < this.correctAnswerSum &&
        this.correctAnswerSum <= responseLevel * 2
      ) {
        this.totalText = quizwa.plug.totalText.middle;
      } else if (this.correctAnswerSum > responseLevel * 2) {
        this.totalText = quizwa.plug.totalText.top;
      }
    },
...
const quizwa = {
  questions: [
    {
      image: "./img/1.jpg",
      text: "Et harum quidem rerum facilis est et expedita distinctio, qui...",
      responses: [
    ...
    {
      image: "./img/2.jpeg",
      text: "Itaque earum rerum hic tenetur a sapiente delectus, quia voluptas...",
      responses: [
        {
          text: "Glossary",
          correct: true,
    ...
    {
      image: "./img/3.png",
      text: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, quae ab...",
      responses: [
    ...
    {
      image: "./img/2.jpeg",
      text: "Sed ut perspiciatis, nam libero tempore, cum soluta nobis est...",
      responses: [
        {
          text: "Какой-то ответ",
          correct: true,
  ...
  plug: {
    heading: {
      start: "Корректность ответов показывается сразу",
    },
    image: {
      start: "./img/start.png",
      end: "./img/end.jpg",
